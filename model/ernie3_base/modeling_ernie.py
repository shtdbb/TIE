...
class ErnieLMPredictionHead(nn.Module):
    ...
    def forward(self, hidden_states):
        hidden_states = self.transform(hidden_states)   # (bs, s, h)
        hidden_states = self.decoder(hidden_states)
        return hidden_states   # (bs, s, 40000)
...
class ErnieForMaskedLM(ErniePreTrainedModel):
    ...
    def forward(
        ...
        sequence_output = outputs[0]   # (bs, s, h)
        prediction_scores = self.cls(sequence_output)   # (bs, s, vocab_size)
